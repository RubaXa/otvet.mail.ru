;define(["Emitter"], function(Emitter) {var src_querystring, src_url, src_match, src_loader, src_request, src_route, src_status, src_pilotjs;
src_querystring = function () {
  var encodeURIComponent = window.encodeURIComponent;
  var decodeURIComponent = window.decodeURIComponent;
  function _stringifyParam(key, val, indexes) {
    /* jshint eqnull:true */
    if (val == null || val === '' || typeof val !== 'object') {
      return encodeURIComponent(key) + (indexes ? '[' + indexes.join('][') + ']' : '') + (val == null || val === '' ? '' : '=' + encodeURIComponent(val));
    } else {
      var pairs = [];
      for (var i in val) {
  ...
  var queryString = /** @lends queryString */
  {
    ...
    parse: function (search) {
      ...
      if (typeof search === 'string') {
        ...
        var pairs = search.trim().split('&'), i = 0, n = pairs.length, pair, name, val;
        for (; i < n; i++) {
          pair = pairs[i].split('=');
...
src_url = function (queryString) {
  ...
  */
  function Url(url, base) {
    ...
    if (url == null) {
      ...
      var protocol = base.protocol, hostname = base.host, pathname = base.pathname;
      if (url.charAt(0) === '#') {
    ...
    this.path = this.pathname = matches[13] || '/';
  ...
  Url.fn = Url.prototype = /** @lends Url# */
  {
    ...
    /** @returns {Url} */
    update: function () {
      this.url = this.href = this.protocol + this.protocolSeparator + (this.username ? encodeURIComponent(this.username) + (this.password ? ':' + encodeURIComponent(this.password) : '') + '@' : '') + this.host + this.pathname + this.search + this.hash;
      return this;
    },
    toString: function () {
      return this.url;
  ...
  */
  Url.toMatcher = function (pattern) {
    ...
    var keys = [];
    pattern = pattern.concat('/*').replace(/\/+/g, '/')  //.replace(/(\/\(|\(\/)/g, '(?:/')
.replace(/\(([^\?])/g, '(?:$1').replace(/(\/)?(\.)?:(\w+)(?:(\([^)]+\)))?(\?)?(\*)?/g, function (_, slash, format, key, capture, optional, star) {
      keys.push({
        name: key,
        optional: !!optional
      });
      slash = slash || '';
      return '' + (optional ? '' : slash) + '(?:' + (optional ? slash : '') + (format || '') + (capture || (format && '([^/.]+)' || '([^/]+)')).replace('(?:', '(') + ')' + (optional || '') + (star ? '(/*)?' : '');
    }).replace(/([\/.])/g, '\\$1');
...
src_loader = function (match) {
  ...
  */
  ...
  Loader.prototype = /** @lends Pilot.Loader# */
  {
    ...
    fetch: function (req) {
      var _index = this._index, names = this.names, models = {}, promises = [], waitFor = function (name) {
          var idx = models[name], model = _index[name];
          if (idx === void 0) {
...
src_request = function (URL, queryString) {
  /**
  * @class  Pilot.Request
  ...
  */
  var Request = function (url, referrer, router) {
    ...
    this.port = url.port;
    this.path = this.pathname = url.pathname;
  ...
  // Export
  return Request;
}(src_url, src_querystring);
src_route = function (Emitter, match, Url, queryString) {
  /**
  ...
  */
  var _urlProcessing = function (rules, target, req) {
    return Object.keys(rules).every(function (name) {
      var rule = rules[name], value = target[name];
      if (value === void 0) {
  ...
  Route.prototype = /** @lends Route# */
  {
    ...
    _initOptions: function (options) {
      ...
      Object.keys(options).forEach(function (key) {
        var value = options[key], act = key.match(/^(one?)[:-](\w+)/);
        if (key === '*') {
          // Регионы
    ...
    match: function (url, req) {
      var params = Url.match(this.url.regexp, url.pathname), query = queryString.parse(url.query), _paramsRules = this.url.params, _queryRules = this.url.query;
      return params && (!_paramsRules || _urlProcessing(_paramsRules, req.params = params, req)) && (!_queryRules || _urlProcessing(_queryRules, req.query = query, req));
    },
    /**
     * Получить данные
     * @param   {Pilot.Request} req
  ...
  */
  ...
  // Export
  return Route;
}(Emitter, src_match, src_url, src_querystring);
src_status = function () {
  /**
  * @class Pilot.Status
...
src_pilotjs = function (Emitter, URL, match, Loader, Request, Route, Status) {
  ...
  */
  ...
  Pilot.prototype = /** @lends Pilot# */
  {
     ...
     */
    nav: function (href) {
      var req, url = new URL(href.toString(), location), _this = this, routes = _this.routes, _promise = _this._promise, currentRoute;
      ...
      if (_this.activeUrl.href !== url.href) {
        ...
        currentRoute = routes.filter(function (item) {
        ...
        if (!_this._promise) {
          ...
          _promise['catch'](function (err) {
            if (currentRoute) {
              ...
              currentRoute.trigger(err.code + '', [
                err,
                req
              ]);
              currentRoute.trigger('error', [
                err,
                req
              ]);
            }
            _this.trigger('route-fail', [
              err,
              req
            ]);
        ...
        if (!currentRoute) {
          ...
          currentRoute.access(req).then(function () {
            ...
            return currentRoute.fetch(req).then(function (model) {
  ...
  */
  ...
  Pilot.version = '2.0.0';
  return Pilot;
}(Emitter, src_url, src_match, src_loader, src_request, src_route, src_status);; return src_pilotjs;});
